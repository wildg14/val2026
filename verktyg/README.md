...
| `forbered_tvaar.py --valnatt-data MAPP` | Bygger testsidan `tmp/tvaar/` (gitignorerad) med två år: 2022 ur `data/` och 2026 ur en valnattskörning. `bakgrund.js`, `swing_2022.js` och `swing_2026.js` är valfria. `--kf-raknade N` låter bara de N första distrikten ha kommunvalet räknat | `.venv/bin/python verktyg/forbered_tvaar.py --valnatt-data /tmp/valnatt-test/data --valnatt --kf-raknade 3 --ut tmp/tvaar-kf` |
| `tvaar-check.js [URL] [KF-URL]` | Kontrollerar tvåårssidan: två årsknappar, 23 distrikt båda åren, Sandarna 2026 och Sandarne 2022, samma viewBox, inga JS- eller konsolfel, och skriver ut vilka kontroller som föll. Med en andra URL till en sida byggd med `--kf-raknade 3` kontrolleras också markörtexten per val | `node verktyg/tvaar-check.js http://localhost:8765/tmp/tvaar/index.html http://localhost:8765/tmp/tvaar-kf/index.html` |
