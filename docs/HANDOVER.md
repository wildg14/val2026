...
Sidvikt (js, css och datafilerna som laddas för ett år): cirka 376 kB, varav bakgrundslagret 228 kB. På valnatten tillkommer `distrikt_2026.js`, `valdata_2026.js` och `swing_2026.js`, tillsammans 52 kB. Miljö: `.venv` med Python 3.12 (uv), paket i `requirements.txt`. Chrome finns på `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome`. Lokal server: `python3 -m http.server 8765 --bind 127.0.0.1` i projektroten, eller förhandsvisningen `valgrafik` i `.claude/launch.json`.
