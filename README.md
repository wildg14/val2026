...
- `samarbete.valvaka.visa` slår på valvakerutan, och kräver att `samarbete.visa` också är på. `hjalp.visa` slår på rösthjälpsrutan för sig; den är påslagen sedan 2026-09-08 och pekar på hjalpmigrosta.se. **Alla länkar grafiken bygger** har `target="_top"`, via konstanten `LANK_MAL` och hjälparen `lankad()`: utan det byter en länk ut grafiken mot den länkade sidan **inuti** Beehiivs iframe i stället för att läsaren lämnar dit, och i praktiken hände ingenting alls vid klick. Uppmätt både i `docs/beehiivtest.html` och på den publicerade sidan, där Beehiivs iframe visade sig sakna `sandbox` och toppfönstret går till målet. Utanför en iframe beter sig `_top` som ett vanligt klick. Orden "fristående, ideell och partipolitiskt obunden" är hämtade ordagrant från hjalpmigrosta.se egen om-sida. Antalet språk i texten (fjorton) är räknat ur sidans egen språkväljare 2026-09-08 - lägger de till fler måste talet ändras för hand. Att Marcus Wernberger Jonsson bor i Majorna och läser Majposten är redaktionens uppgift, inte något som står på hans sida.
- `extra` i en ruta är en avslutande mening som bara visas när rutan är bred nog (`.ruta-extra`, dold som standard). Den ligger i samma stycke som `text`, så meningarna hänger ihop när båda syns. **Gränsen mäter rutans egen bredd, inte sidans:** `.ruta` har `container-type: inline-size`, och frågan är `@container (min-width: 340px)`. En containerfråga mäter innehållsrutan, alltså innanför de 18 px paddingen och ramen. Uppmätt på index: hela texten är 7 rader vid 330 px innehållsbredd och 6 rader från 340 px. Skälet till att sidans bredd inte duger är att rutan står i högerspalten från 900 px och då är 376 px innanför, alltså smalare än sidan antyder. Rösthjälpsrutan ligger under sidhuvudet på mobil och till höger om resultatet från 900 px containerbredd, och visas även när `inbaddad` är på.
...
- `valdag` (`"2026-09-13"`) styr tre saker: om statusraden ska tiga (ett färdigräknat val före valåret säger inget), vilket år som står längst ut på historikens x-axel när räkningen börjat, och vilken indelning som ligger till vänster i konturkartorna.
- `toppsvar.mening` är en redaktionell mening i sidhuvudet, mellan årväljaren och staplarna, tom i `schema.KONFIG_STANDARD` och satt till "Från cirka klockan 20 på valnatten kommer siffrorna löpande här." i `data/konfig.json`. Den står i sitt eget element (`#topp-mening`) och skrivs ut i samma stund som konfigen lästs, alltså före datan, och behöver därför ingen reserverad höjd. Håll den ändå till en rad, cirka 60 tecken.
...
`ar` är åren i väljaren, `standardAr` det år sidan öppnar på. Väljaren är en `<select>` med åren nyast först och etiketterna "Valet 2022", "Valet 2018" och så vidare. Raden är dold när `ar` har färre än två år, och den tar plats så snart konfigen är läst (44 px) så att sidhuvudet inte hoppar.

**Väljaren står på fem ställen**, alla synkade mot samma år: i sidhuvudet (`#arval-select`) och överst i "Om Majorna bestämde", kartsektionen, "Majorna mot Sverige" och "Röstdelningen" (`#arval-select-1` till `-4`). Året gäller hela sidan, och en enda väljare högst upp gjorde det inte tydligt - läsaren såg inte vilket år en sektion längre ned visade. Att i stället låta väljaren följa med vid rullning går inte: blocket ligger i en `iframe srcdoc` hos Beehiiv, där `position: sticky` är verkningslöst, och `position: fixed` är förbjudet av både Beehiivs regler och CSS-lintet. "Majorna sedan 2006" har ingen väljare, eftersom sektionen inte följer året (utom konturkartorna). `renderArval()` bygger alla fem ur `.arval`-behållarna i markupen.
...
Under laddningen står den nuvarande vyn kvar, **alla fem väljare** är `disabled` med `aria-busy` (de visar samma år och får inte gå att sätta i otakt), och fokus läggs tillbaka på den väljare läsaren använde när året är inne. Går året inte att ladda står vyn kvar, alla väljare faller tillbaka på det år som visas, och felraden vid **den väljare som användes** säger "Valet 2014 kunde inte laddas."; varje väljare har en egen `role="status"`-rad, och bara en av dem talar, annars hade felet lästs upp fem gånger. Raden försvinner vid nästa lyckade byte. Ett år som saknar filer tas alltså inte tyst ur väljaren - det går att försöka igen.
...
2006 hör inte hemma i `ar`: `data/distrikt_2006.*` är förenklad geometri för historiksektionens konturkartor. Valåret (2026) hör inte heller dit förrän det har resultat - står det i `ar` laddas dess geometri vid start ändå, och konturkartorna hämtar den då därifrån i stället för som egen fil.
...
- En rubrikmening per val (`#hist-mening`) ur `KONFIG.historik.mening`, redaktionens egen text. Raden är tom och dold när ingen mening är satt. Den räknade meningen om Vänsterpartiets andel första och sista år togs bort 2026-09-08: den sade samma sak som bilden under den.
...
- Konturkartorna under etiketten "Samma yta, olika gränser": **valårets indelning till vänster** (`KONFIG.valdag`, alltså `data/distrikt_2026.js`) och **det år läsaren valt till höger**, som två små `figure`-element, `aria-hidden` eftersom de inte går att interagera med. Är det valda året självt valåret tar 2006 högra platsen, annars hade paret blivit två likadana kartor. Saknas valårets geometri faller paret tillbaka på 2006 till vänster och det visade året till höger. Distrikt per år: 2006 17, 2010 17, 2014 17, 2018 22, 2022 23, 2026 23.
...
Sektionen följer kartans val (riksdag, region eller kommun) men inte årväljaren: sista punkten i valdeltagandebilden kommer alltid från det senast laddade året och ritas bara när alla Majornas distrikt är räknade i det valet - för jämförelseområdena krävs dessutom att området självt är färdigräknat. Konturkartorna följer däremot årväljaren, eftersom de svarar på en fråga om det år kartan visar. **Valåret står på x-axeln först på valnatten.** Före valdagen slutar axeln på det senaste laddade året; 26 stod tidigare längst ut med en tom ring under sig och tog plats utan att säga något. När `KONFIG.valnatt` är sant kommer året tillbaka: är inget distrikt räknat ritas en tom ring på skalans nedersta nivå med texten "räknas just nu", är alla distrikt räknade men resultatet preliminärt blir ringen öppen med en streckad sista sträcka och "(preliminärt)" i beskrivningen, och är resultatet slutligt blir punkten fylld. Texten "räknas på valnatten" står kvar i koden för det läget där valåret är med men ännu inte räknas. Årtalen under axeln skrivs med fyra siffror när avståndet mellan två tick räcker och med två annars - gränsen mäts (`textBredd`), eftersom antalet år följer konfigen.

Sektionen döljs när `data/historik.js` saknas eller `KONFIG.historik.visa` är `false`, vilket stänger av laddningen av `historik.js`, `distrikt_2006.js` och valårets geometri. Alla tre laddas annars vid start - valårets bara när det året inte redan står i `KONFIG.ar`, eftersom `laddaAr` då hämtar det ändå. `swing_<år>.js` hör inte till sektionen utan till de små talen vid staplarna i resultatkortet, och laddas tillsammans med sitt år. En fil som inte går att ladda fångas av sidan: saknas `historik.js` döljs sektionen, saknas `distrikt_2006.js` utgår bara konturkartorna, och saknas `swing_<år>.js` uteblir bara de små talen för det året. (Verktygens undantag för en 404 som en valfri fil gäller bara swingfilerna.)
...
**Sluträkningen tar flera dagar.** Så länge alla 23 distrikt inte är räknade säger statusraden "Slutlig räkning pågår." (talet står i mätaren under) och behåller knappen "Ladda om"; det färdiga beskedet "Slutligt resultat, riksdagsvalet 2026." kommer först när allt är inne. Kör alltså kommandot ovan på samma sätt som under kvällen tills räkningen är klar.
...
Sidhuvudet svarar på frågan innan läsaren scrollar. Ordningen är etikett, rubrik, årväljare, den redaktionella meningen ur konfigen, statusraden och sist toppsvaret: de största partierna i riksdagsvalet som korta staplar under en dämpad rubrik ("Riksdagsvalet 2022 i Majorna"). Alla partier utom Övriga visas i alla bredder. Listan kapades till fyra på telefon fram till 2026-09-08; halva resultatet överst på sidan väckte fler frågor än det svarade på. Eftersom antalet rader inte längre följer bredden ritar ResizeObservern inte om toppsvaret. Banderollen och ingressen som fanns tidigare är borttagna: de sade samma sak två gånger och sköt ned kartan.

**Statusraden tiger för ett färdigräknat val före valåret.** Den sade "Slutligt resultat 2022. Valet 2026 är söndag 13 september.", vilket upprepade årväljaren och den redaktionella meningen ovanför, och togs bort 2026-09-08. Raden bär fortfarande valnattens "Preliminärt. Uppdaterad 21:35." med knappen "Ladda om" (talet står i räknemätaren under raden sedan 2026-09-13), det andra årets "Slutligt resultat 2022. Ladda om", och "Slutligt resultat, riksdagsvalet 2026." när räkningen är klar. `statusradKommer()` avgör ur konfigen, alltså innan datan kommit, om raden ska ta plats: den döljs helt när valåret ligger efter standardåret och `valnatt` är falskt. Samma dag togs valdeltagandemeningen ("82,8 % röstade, mot 84,2 % i riket.") bort ur toppsvaret; valdeltagandet finns kvar som kurva i "Majorna sedan 2006" men inte längre som siffra i sidhuvudet.

Höjderna är reserverade så att sidhuvudet inte hoppar när datan kommer: statusraden 52 px under 600 px containerbredd och 26 px däröver, toppsvaret 314 px i alla bredder (uppmätt naturlig höjd 313,6: rubriken 25,6 plus 6 marginal plus åtta rader 282). Klassen `har-mening` finns inte längre: meningen ligger i sitt eget element utanför toppsvaret och skrivs ut när konfigen lästs, alltså före datan. Varje årväljarrad är 44 px med `margin: 0 0 12px` så fort konfigen listar två år. I nolläget (0 av 23 räknade) visar toppsvaret bara en rad ("Riksdagsvalet 2026: inget distrikt räknat än.") i den reserverade rutan i stället för att krympa den - avsiktligt, så att höjden är densamma före och efter att det första distriktet räknas. "Ladda om" är en `<button>` med länkutseende, inte en länk: den laddar om värdsidan, och en länk hade gått att cmd-klicka till ingenstans.

Resultatkortet visar distriktets namn och staplarna, inget mer i löptext; topp tre-meningen finns bara i skärmläsarraden (`#panel-live`). Förändringen mot förra valet står som små tal vid staplarna, med legendraden "Små tal: förändring mot 2022 i procentenheter" under dem; för hela Majorna får den raden kohorttexten "räknat på N jämförbara distrikt av 23" tills alla är räknade. Ett parti som saknar tal i swingfilen får inget litet tal, aldrig "0,0". Ett omritat distrikt har inga små tal alls och får i stället en fristående mening som säger varför - `ej_jamforbara[kod].mening` ur swingfilen, som för 2026 lyder "Gränserna för Mariaplan ritades om till 2026. Siffrorna går inte att jämföra med 2022." Skärmläsarraden slutar med den meningen. Blocket "Hur har det ändrats", som upprepade de små talen i egna rader, togs bort 2026-09-07, liksom kortets rad med valdeltagande och giltiga röster (valdeltagandet står redan i toppsvaret). Raden är tom och dold sedan 2026-09-13; räkneläget står i mätaren ovanför kartan.
