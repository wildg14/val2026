...
Projektmappen är `/Users/daniel/code/Temp` (git, branch `main`, remote `origin` = GitHub). Arbete sker i grenar under `.claude/worktrees/` som mergas till `main`. Valnattsomgången, historikplanen, alla fyra grupperna ur Daniels feedbackrunda, den andra externa granskningens rättningar och FI i historiken ligger på `main` och är pushade. Kontrollera alltid med `git log origin/main --oneline -1` innan du påstår något om publiceringsläget. Publicering av kodändringar är `git push origin main`; Pages bygger själv inom en minut. Beehiiv-sidan behöver aldrig röras för uppdateringar.

## Arbetsregler som gäller i det här projektet

- **Inga påhittade siffror.** Allt räknas ur datafilerna, och `scripts/kontrollera.py` stämmer av `data/valdata_2022.json` mot `majorna-valresultat-2022.xlsx` (1 149 kontroller). Ändra aldrig ett tal för hand. Kör `.venv/bin/python -m pytest -q` (**420 tester**, alla gröna och inga överhoppade i den här mappen; i en klon utan Valmyndighetens råfiler, genrep-filerna och pem-nycklarna hoppas några över - saknas bara pem-filerna blir det 418 gröna och 2 överhoppade) före och efter ändringar.
...
| Tester | `tests/` | 420 tester (`pytest --collect-only -q`, uppmätt 2026-09-09), fördelade på funktionsnivå: `test_bygg_historik.py` 78, `test_inbaddning.py` 69, `test_valnatt.py` 54, `test_uppdatera.py` 54, `test_hamta_2026.py` 49, `test_geo.py` 39, `test_forbered_tvaar.py` 26, `test_schema.py` 19, `test_skapa_bilder.py` 9, `test_kontrollera.py` 9, `test_valmyndigheten.py` 6, `test_bakgrund.py` 4, `test_mandat.py` 3, `test_bygg_data.py` 1 (parametriserade fall gör att summan av körda tester är högre per fil). Innehåll: mandatberäkning, parser mot råfiler, geometri (2022 och 2026, byte-identitet mot de committade filerna), schema och swing, kontrollskript, bygge, bakgrund, uppdatera (JSON-, xlsx- och CSV-vägarna, spärrarna), hämtning och signaturer, valnattsmappningen mot genrep-filerna, stillbilder, Beehiiv-regler och sidans egen kod (`test_inbaddning.py`: scroll-margin-top, `sidLocation`, `stickyTopp`, samarbete/hjälp-nycklarna, och sedan feedbackrundan den lata laddningen, årväljarens select, toppsvarets partiantal, kortets små tal, kartan utan partibokstäver, bild A:s frånvaro, konturkartornas not, sedan grupp 3 mandattabellens behållare, brytpunkten i CSS, enhetsväxeln, procentlägets kolumnrubriker och den uppmätta gallringen av axeletiketterna, och sedan grupp 4 den tysta statusraden, sidhuvudets ordning, väljaren i varje sektion, valåret på historikaxeln och toningen av omritade distrikt, och sedan den andra granskningen tabellens fokusåterställning, statusradens gren för pågående sluträkning, bildramens kvitto och FI i halvcirkelns ordning), historikbygget (`test_bygg_historik.py`, inklusive FI som eget parti till och med 2018), `forbered_tvaar.py` (`test_forbered_tvaar.py`: `--kf-raknade`, `--partiell`, `--status`, `--utan-parti`, `--riket-delvis` och felvägarna). |
...
Historikens facit, sedan FI blev eget parti i riksdagsvalet till och med 2018 (räknat ur `data/valdata_<år>.json` och `data/historik.json`). FI i riksdagsvalet, Majorna mot Göteborg och riket: 2006 2,50 mot 1,26 och 0,68 %, 2010 2,10 mot 0,86 och 0,40 %, **2014 16,50 mot 6,48 och 3,12 %**, 2018 1,82 mot 0,84 och 0,46 %, 2022 0,11 mot 0,09 och 0,05 % (2022 ligger kvar i Övriga). Riksdagsvalet 2014 i Majorna: V 19,6 %, S 17,8 %, FI 16,5 %, MP 14,7 %, M 12,3 %, SD 7,4 %, L 5,1 %, C 2,8 %, KD 2,3 %, Övriga 1,5 %, av 20 960 giltiga röster. "Om Majorna bestämde" 2014: V 73, S 66, FI 62, MP 55, M 46, SD 28, L 19, C och KD under spärren; verklig riksdag 2014 S 113, M 84, SD 49, MP 25, C 22, V 21, L 19, KD 16, FI 0.
...
18. **Samarbetsblock och rösthjälp förberedda.** En rad "I samarbete med Majornas Bryggeri" i sidhuvudet och upp till två rutor (valvaka, hjalpmigrosta.se) direkt under, allt styrt av `samarbete`/`hjalp` i `data/konfig.json`. **Ändrat 2026-09-08:** rösthjälpsrutan är påslagen (`hjalp.visa: true`) och pekar på hjalpmigrosta.se, en fristående, ideell och partipolitiskt obunden valguide gjord av Marcus Wernberger Jonsson. Texten är kontrollerad mot sidans egen om-sida och mot dess "Hur röstar jag": fjorton språk räknade ur språkväljaren, och guiden tar upp röstkortet, valsedlarna, förtidsröstningen och vallokalen. Samarbetsraden och valvakerutan är fortfarande avstängda, med Daniels platshållartexter. Texten nämner att han är Majornabo och Majpostenläsare; det är redaktionens uppgift, inte något som står på hans sida, och står här så att nästa session vet varifrån den kommer. Rutans avslutande mening ligger i konfignyckeln `extra` och visas först när rutan själv är bred nog: `.ruta` har `container-type: inline-size` och frågan är `@container (min-width: 340px)`, mätt på innehållsrutan innanför paddingen. Hela texten är 7 rader vid 330 px och 6 rader från 340 px. Sidans bredd duger inte som mått, eftersom rutan står i högerspalten från 900 px och då är 376 px innanför. **Alla** länkar grafiken bygger fick samtidigt `target="_top"`, via konstanten `LANK_MAL` och `lankad()`: rösthjälpsrutan var den första länken grafiken hade i standardläget, och utan `_top` hände ingenting alls vid klick inuti Beehiivs iframe. Uppmätt först i `docs/beehiivtest.html` och sedan på den publicerade sidan, där Beehiivs iframe visade sig sakna `sandbox` och toppfönstret går till hjalpmigrosta.se. Samarbetsraden och loggan hade samma lucka, fast avstängda. Samma dag kortades källraden i "Om siffrorna" till bara `meta.kalla`: ordet slutlig eller preliminär står redan där, så statusmeningen var en upprepning, och andelsdefinitionen togs bort på Daniels begäran. Avslutande punkter och blanksteg i `kalla` skalas av innan raden skrivs, eftersom historikårens filer slutar med punkt och 2022 års inte - annars stod det "per valdistrikt 2018.." i väljarens historiska år. En tom `kalla` ger ingen rad alls i stället för "Källa: ." med bara en punkt.
...
10. Toppsvaret visar fyra partier under 600 px containerbredd och alla partier utom Övriga från 600 px. **Upphävt i grupp 4 2026-09-08:** alla partier utom Övriga visas i alla bredder.
...
Grupp 3 är resten av samma feedbackrunda: Daniels önskemål om en växel mandat eller procent i "Om Majorna bestämde" ("gemeneman tänker sällan i mandat, mer i %"), plus de två layoutfynden som slutgranskningen av historiksektionen hade lagt i startpunkten. Två commits, `41adb23` och `18fab59`, byggda på grenen `claude/historiksektion-handover-b6b0b1` och sedan mergade till `main` tillsammans med dokumentationscommiten `9f1b6f8`. Allt är pushat - kontrollera med `git log origin/main --oneline -1`.
...
1. Statusraden tiger för ett färdigräknat val som ligger före valåret. Meningen "Slutligt resultat 2022. Valet 2026 är söndag 13 september." upprepade årväljaren och den redaktionella meningen. Valnattens räknestatus med "Ladda om", det andra årets rad och "Slutligt resultat, riksdagsvalet 2026." står kvar. `statusradKommer()` avgör ur konfigen, alltså före datan, om raden ska ta plats alls, så att den inte lämnar ett tomt band.
2. Valdeltagandemeningen "82,8 % röstade, mot 84,2 % i riket." borttagen ur toppsvaret. Talet finns inte längre någonstans på sidan som siffra - bara som kurva i "Majorna sedan 2006". Kortets motsvarande rad togs bort redan i grupp 1, med motiveringen att talet stod i toppsvaret.
...
**Räkneexemplet under halvcirkeln** är 13 px, samma grad som mandattabellens kolumnrubriker. Ingen asterisk sattes dit: ett fotnotstecken utan ankare någon annanstans blir en lös ände.

**Historiksektionen.** Valåret står på x-axeln först på valnatten - före valdagen tog 26 plats längst ut med en tom ring under sig utan att säga något. Årtalen skrivs med fyra siffror när avståndet mellan två tick räcker, mätt med `textBredd` och inte satt som brytpunkt, eftersom antalet år följer konfigen. Den räknade meningen "V har gått från 17,2 till 27,2 procent i riksdagsvalet sedan 2006." är borta; `histMening` returnerar nu bara redaktionens egen mening ur konfigen, och raden är dold när den är tom. Ledarlinjen från punkt till isärskjuten etikett i valdeltagandebilden är borttagen - Daniel kallade den "den lilla flärpen som hänger nedåt".

**Konturkartorna gjordes om två gånger.** Först ställdes valårets indelning mot det valda året (Daniels idé), men 2026 mot 2022 blev två likadana teckningar: ytan är identisk och de elva distrikt som fått nya gränser flyttar bitar som inte syns i 200 px bredd. Lösningen blev att tona de omritade distrikten i båda kartorna. `jamforDistrikt()` avgör det på distriktskod, `area_km2` och omskrivande rektangel - polygonerna räknas om mellan åren och får både andra hörnantal och annan startpunkt, så formen kan inte jämföras hörn för hörn. År utan gemensamma koder (2006, 2010, 2014 och 2018 mot 2026) tonas inte alls. Är det valda året självt valåret tar 2006 högra platsen. Etiketten lyder "Samma yta, olika gränser" och noten namnger båda åren. Meningen "Hur olika åldrar röstade går inte att veta - valhemligheten gäller per distrikt, inte per person." togs bort ur noten samma dag på Daniels begäran; ett test vaktar att den inte kommer tillbaka. Uppgiften finns kvar i `docs/historik/` för den som behöver den.

**Granskningsupplägget.** Tre agenter: en specgranskare på Sonnet mot Daniels tio beslutspunkter, en kvalitetsgranskare på Opus mot diffen, och en motgranskare på Sonnet som reproducerade varje fynd innan det räknades. Specgranskaren hittade ingen avvikelse från besluten. Av kvalitetsgranskarens åtta fynd bekräftades fyra och alla fyra rättades:

1. **Important:** bara sidhuvudets väljarrad fick sin plats reserverad vid start. De fyra i sektionerna stod `hidden` från markupen tills `renderArval` körde efter datan, och eftersom sektionerna syns från början med sina rubriker hoppade sidan cirka 224 px när datan landade. `start()` går nu igenom `arvalRutor()`. Uppmätt efteråt med fördröjda datafiler: alla fem rader står på plats före datan, och `#riksdag` ligger på 427 px både före och efter.
2. **Minor:** `jamforDistrikt` jämförde area med `|| 0`. En årsfil utan `area_km2` hade gjort varje distrikt omritat och noten hade påstått att hela indelningen ändrats. Arean jämförs nu bara när båda filerna har talet, annars avgör den omskrivande rektangeln ensam.
3. **Minor:** den här filen beskrev fortfarande läget före grupp 4 på tre ställen (beslut 29, grupp 2 punkt 7 och en fallgrop om sidhuvudets höjd). Rättat.
4. **Minor:** noten skrev "De N tonade distrikten", vilket blev "De 1 tonade distrikten" vid ett distrikt och dessutom dolde att samma distrikt tonas i **båda** kartorna. Noten lyder nu "Elva distrikt har fått nya gränser och är tonade i båda kartorna", med små tal utskrivna i löptext (`rakneord`).
...
## Status 2026-09-13: räknemätaren och valnattsslingan

Natten till valdagen. **Räknemätaren** (spec `docs/superpowers/specs/2026-09-13-raknemataren-design.md`, plan `docs/superpowers/plans/2026-09-13-raknemataren.md`): två staplar under statusraden, "Majorna N av 23 distrikt räknade" och "Sverige N av 6 626", och en ovanför kartan som följer valknapparna - `matarRad`, `matareVisas` och `renderMatare` i `valgrafik.js`, CSS-klasserna `.matare*`. Visas i samma läge som ger statusraden "Ladda om" för det levande året, står fulla när allt är räknat och försvinner när `valnatt` slås av. Daniels beslut: statusraden kortades till "Preliminärt. Uppdaterad HH:MM." och "Slutlig räkning pågår.", och kortets underrad "N av 23 distrikt räknade." togs bort - talet står på ett ställe. Kontrollerna i `tvaar-check`, `historik-check` och `skal-check` läser mätaren i stället för raden; `test_inbaddning` har ett test för mätaren. Höjden 90 px är uppmätt (två rader, samma i 320, 375 och 1280 px).

**Valnattsslingan** `verktyg/valnatt-slinga.sh` kör kvällen utan handpåläggning som launchd-agent (`verktyg/se.majposten.valnatt.plist`), se körschemats avsnitt Slingan. Daniels beslut samma natt: ingenting görs för hand under valnatten. Val.se tog bort genrepet 2026-09-12 (404) och `val2026/index.md5` svarar 200 med en tom lista fram till valkvällen; körschemats torrkörning går därför lokalt mot `Historiska dokument/dl_webb/genrep2026/`.
...
- **Tillåtlistan är beslutad:** `document.visibilityState` får läggas till i `DOCUMENT_TILLATNA` i `tests/test_inbaddning.py` (i dag `currentScript`, `createElement`, `createElementNS`, `createTextNode`, `head` och `getElementsByClassName`). Den är läsbar och rör inte värdsidan. Utöka listan medvetet och skriv varför i testet.
- **Raden ska inte trängas in i sidhuvudet utan eftertanke.** Sidhuvudets höjder är uppmätta (se grupp 4), och en rad som dyker upp när nya siffror finns flyttar allt under sig. Reservera höjd på samma sätt som årväljarens rad, eller lägg raden där statusraden står - den är ändå tom före valdagen.
...
- **Årväljaren finns på fem ställen.** `renderArval()` bygger alla `.arval`-behållare i markupen, och `byteAr(ar, valj)` tar den select som användes som andra argument för att kunna lägga tillbaka fokus där. Lägger du till en sektion som följer året lägger du dit en `<div class="arval" hidden></div>` och en `<p class="arval-fel" role="status" hidden></p>` direkt efter den; felraden hittas som `nextElementSibling`. Selecternas id är `arval-select` och `arval-select-<index>` efter ordningen i DOM, så sidhuvudets måste ligga först - `tvaar-check.js` och `historik-check.js` väljer år via `#arval-select`.
- **Statusraden döljs ur konfigen, inte ur datan.** `statusradKommer()` läser `KONFIG.valnatt`, `KONFIG.valdag` och `KONFIG.standardAr` och sätter `hidden` i `start()`, innan datan kommit. Flyttar man den bedömningen till `renderToppsvar()` reserveras 52 px som sedan försvinner, och sidhuvudet hoppar. `renderToppsvar()` sätter `hidden` en gång till när texten faktiskt är tom, vilket är rätt: konfigen kan säga att raden kommer att tala i något läge, medan just den vyn är tyst.
