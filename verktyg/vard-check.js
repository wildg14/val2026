...
(async () => {
  ...
  const res = await page.evaluate(() => {
    const cs = el => { const c = getComputedStyle(el); return { color: c.color, font: c.fontFamily.split(',')[0], transform: c.textTransform, deco: c.textDecorationLine + ' ' + c.textDecorationStyle, radius: c.borderRadius, bg: c.backgroundColor }; };
    const rot = document.getElementById('valgrafik');
    return {
      vara: { h2: cs(rot.querySelector('#riksdag-rubrik')), knapp: cs(rot.querySelector('#flikar button')), lank: cs(rot.querySelector('#fot a') || rot.querySelector('a')), li: cs(rot.querySelector('.legend li')), td: cs(rot.querySelector('table.mandat td')) },
